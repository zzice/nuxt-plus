<template>
  <header
    class="sticky top-0 z-50 w-full border-b border-border/20 bg-background/80 backdrop-blur-xl supports-[backdrop-filter]:bg-background/60 transition-all duration-300">
    <!-- 顶部装饰线 -->
        ...
        <div class="relative flex flex-wrap items-center justify-between gap-6 py-4 lg:gap-0 lg:py-5">
          ...
          <div class="flex w-full items-center justify-between gap-12 lg:w-auto">
            ...
            <NuxtLinkLocale to="/" aria-label="home"
              ...
              <div class="relative">
                <div
                  class="w-10 h-10 rounded-2xl bg-gradient-to-br from-primary to-secondary flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all duration-300">
                  <img src="/logo.png" alt="Logo" class="size-6 rounded-md" />
                </div>
                <div
                  class="absolute inset-0 bg-gradient-to-br from-primary to-secondary rounded-2xl blur-md opacity-30 group-hover:opacity-50 transition-opacity duration-300 -z-10">
                </div>
              </div>
              <span
                class="font-bold text-xl bg-gradient-to-r from-foreground to-foreground/80 bg-clip-text text-transparent group-hover:from-primary group-hover:to-secondary transition-all duration-300">NuxtPlus</span>
            </NuxtLinkLocale>

            <!-- 移动端菜单按钮 -->
            <button @click="toggleMobileMenu" :aria-label="isMobileMenuOpen ? 'Close Menu' : 'Open Menu'"
              ...
              <ul class="flex gap-2 text-sm">
                <li v-for="link in navLinks" :key="link.href">
                  <NuxtLinkLocale :to="link.href"
                    ...
                    <div
                      class="absolute inset-0 bg-gradient-to-r from-primary/10 to-secondary/10 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    </div>
                  </NuxtLinkLocale>
                </li>
              </ul>
          ...
          <!-- 右侧功能区和移动端菜单 -->
          <div
            class="bg-background/95 backdrop-blur-xl group-data-[state=active]:block lg:group-data-[state=active]:flex mb-6 hidden w-full flex-wrap items-center justify-end space-y-8 rounded-3xl border border-border/20 p-6 shadow-2xl shadow-primary/10 md:flex-nowrap lg:m-0 lg:flex lg:w-fit lg:gap-6 lg:space-y-0 lg:border-transparent lg:bg-transparent lg:p-0 lg:shadow-none">
            <!-- 移动端导航菜单 -->
            <div class="lg:hidden">
              <ul class="space-y-4 text-base">
                <li v-for="link in navLinks" :key="link.href">
                  <NuxtLinkLocale :to="link.href"
                    ...
                    <span class="relative z-10">{{ $t(link.name) }}</span>
                    <div
                      class="absolute inset-0 bg-gradient-to-r from-primary/5 to-secondary/5 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    </div>
                  </NuxtLinkLocale>
                </li>
              </ul>
            ...
            <!-- 登录/注册按钮或用户信息 -->
            <div class="flex w-full flex-col space-y-4 sm:flex-row sm:gap-4 sm:space-y-0 sm:items-center md:w-fit">
              <!-- 语言和主题切换 -->
              ...
              <!-- 未登录状态 -->
              <div v-if="!user" class="flex flex-col sm:flex-row gap-3 items-center">
                <NuxtLinkLocale to="/login">
                  <Button class="group relative overflow-hidden transition-all duration-300 hover:scale-105">
                    ...
                    <div
                      class="absolute inset-0 bg-gradient-to-r from-primary/90 to-secondary/90 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    </div>
                  </Button>
                </NuxtLinkLocale>
              </div>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger class="focus:outline-none group">
                    <div class="flex items-center p-1 rounded-xl hover:bg-primary/10 transition-all duration-300">
                      <div v-if="defaultAvatar.type === 'image'"
                        class="h-10 w-10 rounded-full overflow-hidden ring-2 ring-transparent group-hover:ring-primary/50 transition-all duration-300">
                  ...
                  <DropdownMenuContent class="w-64 p-2 bg-background/95 backdrop-blur-xl border-border/20">
                    <DropdownMenuLabel>
                      <div class="flex flex-col py-2">
                        <p
                          class="text-base font-semibold bg-gradient-to-r from-foreground to-foreground/80 bg-clip-text text-transparent">
                    ...
                    <NuxtLinkLocale to="/dashboard">
                      <DropdownMenuItem
                        class="py-3 cursor-pointer rounded-lg hover:bg-primary/10 transition-all duration-300 group">
                    ...
                    <DropdownMenuItem @click="handleSignOut"
                      class="py-3 cursor-pointer rounded-lg hover:bg-destructive/10 transition-all duration-300 group">
                      <LogOut class="w-4 h-4 mr-3 group-hover:text-destructive transition-colors duration-300" />
                      <span class="text-sm font-medium group-hover:text-destructive transition-colors duration-300">{{
                        $t('auth.signOut') }}</span>
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
